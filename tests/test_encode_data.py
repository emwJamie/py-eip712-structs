...
def test_encode_nested_structs():
    ...
    s = MainStruct(sub_1=sub_1, sub_2=s2, sub_3=sub_3,)

    expected_encoded_vals = b"".join(
        [sub_1.hash_struct(), keccak(text=s2), sub_3.hash_struct()]
    )
...
def test_data_dicts():
    ...
    bar = Bar(foo=Foo(s="hello", i=100,), b=b"\xff")

    expected_result = {"foo": {"s": "hello", "i": 100,}, "b": b"\xff"}
